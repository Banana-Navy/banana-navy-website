...
import { articleTakeaways, labArticles, locationContexts, locationNames, locations, serviceCategories, tr, type Lang } from "./site-data";
...
function Header({ lang }: { lang: Lang }) {
  ...
  return <header className="topbar inner-topbar">
    ...
    <div className="header-actions"><div className="langs">{(["fr","nl","en"] as Lang[]).map(l=><Link className={l===lang?"active":""} href={`/${l}`} key={l}>{l.toUpperCase()}</Link>)}</div><a className="pill" href="mailto:marc@banana-navy.com">{t.contact} ↗</a><details className="internal-menu"><summary aria-label="Menu">≡</summary><nav className="internal-menu-panel" aria-label="Mobile navigation"><Link href={`/${lang}/expertise`}>{t.expertise}</Link><Link href={`/${lang}/approach`}>{t.approach}</Link><Link href={`/${lang}/lab`}>{t.lab}</Link><Link href={`/${lang}/locations`}>{t.locations}</Link><Link href={`/${lang}/team`}>{t.team}</Link></nav></details></div>
  </header>;
}
...
export function getPageInfo(lang: Lang, slug: string[]) {
  ...
  if ((first==="legal" || first==="privacy") && !second) return { title:tr(legalPages[first].title,lang), description:tr(legalPages[first].description,lang), type:first };
...
export default function InternalPage({ lang, slug }: { lang: Lang; slug: string[] }) {
  ...
  return <main className="internal-page" lang={lang}><Header lang={lang}/>
    ...
    {info.type==="article" && info.article && <article className="article-body"><div className="article-meta"><span>{info.article.topic}</span><time>{info.article.date}</time><span>Banana Navy Lab</span></div><h2>{info.description}</h2><p>{lang==="fr"?"La bonne question n'est pas de savoir si la technologie est impressionnante, mais si elle produit une amélioration observable sans déplacer le risque ailleurs. Nous examinons donc chaque sujet sous l'angle de l'usage, des données, des opérations et de la reprise humaine.":lang==="nl"?"De juiste vraag is niet of de technologie indrukwekkend is, maar of ze een meetbare verbetering oplevert zonder het risico te verplaatsen. Daarom bekijken we elk onderwerp vanuit gebruik, data, operations en menselijke overname.":"The right question is not whether the technology is impressive, but whether it creates an observable improvement without moving risk elsewhere. We therefore examine each topic through usage, data, operations and human takeover."}</p><h3>{lang==="fr"?"Trois points à retenir":lang==="nl"?"Drie aandachtspunten":"Three practical takeaways"}</h3><ul className="article-points">{articleTakeaways[info.article.slug][lang].map(point=><li key={point}>{point}<i>✓</i></li>)}</ul><h3>{t.method}</h3><p>{t.methodText}</p><div className="article-callout"><b>Banana Navy principle</b><p>{lang==="fr"?"Livrer petit, mesurer en conditions réelles, documenter les limites, puis étendre ce qui fonctionne.":lang==="nl"?"Klein leveren, meten in echte omstandigheden, grenzen documenteren en daarna uitbreiden wat werkt.":"Ship small, measure in real conditions, document limits, then expand what works."}</p></div><Link href={`/${lang}/lab`} className="text-link">← {t.back} · {t.lab}</Link></article>}
    ...
    {(info.type==="legal" || info.type==="privacy") && <section className="content-section legal-copy">{legalPages[info.type].sections[lang].map(([heading,body])=><article key={heading}><h2>{heading}</h2><p>{body}</p></article>)}</section>}
